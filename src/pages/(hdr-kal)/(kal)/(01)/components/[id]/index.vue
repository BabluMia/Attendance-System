<template>
  <div class="p-4">
    ...
    <div class="mt-6 flex justify-end space-x-4">
      ...
      <DxButton text="Update" type="success" stylingMode="contained" @click="updateCalendar" />
    </div>
  </div>
</template>
...
// Form data initialized with default values
...
const updateCalendar = () => {
  try {
    ...
    const updatedCalendarData = { ...formData.value };

    if (updatedCalendarData.restBreak) {
      updatedCalendarData.restBreak.startDate = formatDate(updatedCalendarData.restBreak.startDate);
      updatedCalendarData.restBreak.endDate = formatDate(updatedCalendarData.restBreak.endDate);
    }

    if (updatedCalendarData.termBreaks) {
      updatedCalendarData.termBreaks.forEach((term) => {
        term.startDate = formatDate(term.startDate);
        term.endDate = formatDate(term.endDate);
      });
    }

    if (updatedCalendarData.holidayEvents) {
      updatedCalendarData.holidayEvents.forEach((event) => {
        event.date = formatDate(event.date);
      });
    }

    calendarStore.update(updatedCalendarData);
    notify({
...
onMounted(() => {
  // Get the calendar data by ID
  const calendar = calendarStore.getById(calendarId);
